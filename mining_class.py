...
class AllMines():
    ...
    def generate_df(self):
        return pd.concat([self.loc[q].generate_df() for q in self.loc],keys=self.loc)

class OneMine():
    def __init__(self, name, df=None):
        self.name = name
        if type(df)!=type(None):
            new_strs = [str(i).lower().replace(' ','_').replace('(','').replace(')','').replace('$','usd').replace('/','p').replace('%','pct') for i in df.columns]
    ...
    def sample(self,size=1,resample=False):
        if resample:
    ...
    def generate_df(self):
        return pd.DataFrame([getattr(self,q) for q in self.columns],columns=self.index,index=self.columns).T
...
#         self.incentive_tuning_option = self.hyperparam['Value']['incentive_tuning_option']
    ...
    def plot_relevant_params(self,include=0,exclude=[],plot_recovery_grade_correlation=True,plot_minesite_supply_curve=True, plot_margin_supply_curve=True,plot_primary_minesite_supply_curve=False, plot_primary_margin_supply_curve=False,log_scale=False,dontplot=False,byproduct=False):
        ...
        for i,a in zip(cols,ax):
            ...
            if self.byproduct and i not in ['Byproduct ID','Byproduct cash flow ($M)']:
                ...
                mines[i].replace({'Primary':0,'Host 1':1,'Host 2':2,'Host 3':3}).plot.hist(ax=a, title=i.replace('Primary','Host'), bins=50)
        ...
        if plot_primary_minesite_supply_curve and self.byproduct:
            ...
            minn,maxx = min(minn,mines['Primary Commodity price (USD/t)'].min()),max(maxx,mines['Primary Commodity price (USD/t)'].max())
            supply_curve_plot(mines,'Primary Production (kt)',['Primary Total cash cost (USD/t)'],
                              price_col='Primary Commodity price (USD/t)',width_scale=1.2,line_only=True,ax=a,xlabel='Cumulative production (kt)',
    ...
    def initialize_hyperparams(self,hyperparam):
        ...
        if type(hyperparam)==int:
            ...
            if 'parameters for operating mine pool generation, cost':
                ...
                hyperparameters.loc['primary_overhead_regression2use',:] = 'linear_194','options are linear_194, which uses values from 20210825 Generalization.pptx slide 194 or None, which gives a constant value for overhead at all mines of 0.1, in $M'
            ...
            if 'parameters for mine life simulation':
                ...
                hyperparameters.loc['cu_cutoff',['Value','Notes']] = np.array([1.1,'highest allowable capacity utilization'],dtype='object')
                hyperparameters.loc['ramp_down_cu','Value'] = 0.4
                hyperparameters.loc['ramp_up_cu','Value'] = 0.4 # currently replacing this s.t. linear ramp up to 100% instead
                ...
                hyperparameters.loc['load_mine_life_init_from_pkl',:] = np.array([False,'self explanatory'],dtype=object)
    ...
    #             hyperparameters.loc['',['Value','Notes']] = np.array([],dtype='object')
            ...
            if 'parameters for the incentive pool':
                ...
                hyperparameters.loc['incentive_roi_years',:] = 10, 'Number of years of mine life to simulate to determine profitability for opening mines'
                hyperparameters.loc['incentive_opening_method',:] = 'unconstrained', 'options: xinkai_thesis, karan_generalization, unconstrained; xinkai_thesis is the tuning of the incentive subsample size to match demand, while karan_generalization is tuning of price to fill the supply-demand gap. unconstrained allows mine opening to occur unimpeded and TCRC evolves based on TCRC elasticity to SD imbalance to close the gap.'
                hyperparameters.loc['calibrate_incentive_opening_method',:] = False, 'whether to calibrate to minimize the demand gap if using the xinkai_thesis incentive_opening_method'
                hyperparameters.loc['demand_series_method',:] = 'yoy', 'Options: yoy or target; yoy is year-over-year percent change in demand, target is the percent change in demand by simulation end.'
                hyperparameters.loc['demand_series_pct_change',:] = 0.5, 'Percent change per year if demand_series_method is yoy, or percent change from initial to last simulation year if demand_series_method is target'
            ...
            if hyperparameters['Value']['demand_series_method']=='yoy':
                hyperparameters.loc['demand_series',:] = np.array([pd.Series([initial_demand*change**(j-sim_time[0]) for j in sim_time],sim_time),hyperparameters['Notes']['demand_series']],dtype=object)
            ...
            raise ValueError('Mining module initialization failed, incorrect hyperparam input (should be integer/zero to allow self-generation, string for filepath (excel or pickle file), or dataframe if it already exists)')
        self.mine_cu_margin_elas, self.mine_cost_og_elas, self.mine_cost_price_elas, self.mine_cu0, self.mine_tcm0, self.discount_rate, self.ramp_down_cu, self.ramp_up_cu, self.ramp_up_years = \
            self.hyperparam['Value'][['mine_cu_margin_elas','mine_cost_og_elas','mine_cost_price_elas',
                                 'mine_cu0','mine_tcm0','discount_rate','ramp_down_cu','ramp_up_cu','ramp_up_years']]
        ...
        if type(innie)==int:
            ...
            if self.byproduct:
                ...
                for i in np.arange(1,4):
                    ...
                    hyperparameters.loc['byproduct'+str(i)+'_rr0','Value'] = np.median(mines.recovery_rate_pct[(mines.byproduct_id==i)&(getattr(mines,byp+'total_cash_margin_usdpt')>0)])
                    hyperparameters.loc['byproduct'+str(i)+'_mine_tcm0','Value'] = np.median(getattr(mines,byp+'total_cash_margin_usdpt')[(mines['Byproduct ID']==i)&(getattr(mines,byp+'total_cash_margin_usdpt')>0)])
...
#                 + delta*log(capacity) + epsilon*placer + theta*stockpile + eta*tailings + rho*underground + zeta*sxew
    ...
    def recalculate_hyperparams(self):
        ...
        if hyperparameters['Value']['demand_series_method']=='yoy':
            ...
            hyperparameters.loc['demand_series',:] = np.array([pd.Series(np.linspace(initial_demand,initial_demand*change,len(sim_time)),sim_time),hyperparameters['Notes']['demand_series']],dtype=object)
            self.demand_series = hyperparameters['Value']['demand_series']
        elif self.verbosity>1: print('using whatever value has already been assigned to demand_series')
    ...
    def generate_production_region(self):
        ...
        mines.loc[:,'Production fraction'] /= pri_prod

        mines_not_ramping = mines.copy()
        mines.loc[:,'Ramp up flag'] = 0
        if h['ramp_up_fraction']!=0:
            for i in np.arange(1,h['ramp_up_years']+1):
                ...
                ind = mines_not_ramping.sample(num_ramping,replace=False).index
                mines_not_ramping.drop(ind,inplace=True)
                mines.loc[ind,'Ramp up flag'] = i
                if h['ramp_up_exponent']!=0:
                    ramp_up_exp = h['ramp_up_exponent']
                    mines.loc[ind,'Production fraction'] *= (h['ramp_up_cu']+(1-h['ramp_up_cu'])/(h['ramp_up_years'])**ramp_up_exp*(mines.loc[ind,'Ramp up flag']-1)**ramp_up_exp)
                else:
        ...
        # mines.production_fraction = np.append(mines.production_fraction, [production_fraction-np.sum(mines.production_fraction)])
        ...
        mines.loc[:,'Simulation start ore treated (kt)'] = np.nan
        mines.loc[:,'Ramp down flag'] = False
        mines.loc[:,'Closed flag'] = False
        mines.loc[:,'Operate with negative cash flow'] = False
        mines.loc[:,'Total cash margin expect (USD/t)'] = np.nan
        mines.loc[:,'NPV ramp next ($M)'] = np.nan
        mines.loc[:,'NPV ramp following ($M)'] = np.nan
        mines.loc[:,'Close method'] = np.nan
        mines.loc[:,'Simulated closure'] = np.nan
        mines.loc[:,'Initial head grade (%)'] = np.nan
        mines.loc[:,'Discount'] = 1
        mines.loc[:,'Initial price (USD/t)'] = np.nan
    ...
    def generate_grade_and_masses(self):
        ...
        if h['ramp_up_exponent']!=0:
            ramp_up_exp = h['ramp_up_exponent']
            mines.loc[ind,'Capacity utilization'] = h['mine_cu0']*(h['ramp_up_cu']+(1-h['ramp_up_cu'])/(h['ramp_up_years'])**ramp_up_exp*(mines.loc[ind,'Ramp up flag']-1)**ramp_up_exp)
        else:
            mines.loc[ind,'Capacity utilization'] = h['ramp_up_cu']
        mines.loc[:,'Production capacity (kt)'] = mines['Capacity utilization']*mines['Production (kt)']
...
#             partial_shuffle(np.sort(rec_rates),self.hyperparam['Value']['primary_recovery_rate_shuffle_param'])
        ...
        if h['ramp_up_exponent']!=0:
            ...
            for i in np.arange(1,h['ramp_up_years']+1):
                ...
                mines.loc[ind2,'Cumulative ore treated (kt)'] += mines.loc[ind2,'Ore treated (kt)'] * (h['ramp_up_cu']+(1-h['ramp_up_cu'])/(h['ramp_up_years'])**ramp_up_exp*(i-1)**ramp_up_exp) / (h['ramp_up_cu']+(1-h['ramp_up_cu'])/(h['ramp_up_years'])**ramp_up_exp*(mines.loc[ind2,'Ramp up flag']-1)**ramp_up_exp)
            mines.loc[ind,'Initial ore treated (kt)'] = mines.loc[ind,'Ore treated (kt)'] * h['ramp_up_cu'] / (h['ramp_up_cu']+(1-h['ramp_up_cu'])/(h['ramp_up_years'])**ramp_up_exp*(mines.loc[ind,'Ramp up flag']-1)**ramp_up_exp)
        else:
            mines.loc[ind,'Cumulative ore treated (kt)'] = mines.loc[ind,'Ore treated (kt)']*mines.loc[ind,'Ramp up flag']
        ...
        mines.loc[ind,'Cumulative ore treated ratio with ore treated'] = mines['Cumulative ore treated (kt)'][ind]/mines['Ore treated (kt)'][ind]
...
#                 + delta*log(ore treated) + epsilon*placer + theta*stockpile + eta*tailings + rho*underground + zeta*sxew
    ...
    def assign_mine_types(self):
        ...
        mines.loc[mines['Mine type'].isna(),'Mine type'] = self.mine_type_mapping_rev[h[params].astype(float).idxmax().split('_')[-1]]
        mines.loc[mines['Mine type string'].isna(),'Mine type string'] = h[params].astype(float).idxmax().split('_')[-1]
        self.mines = mines.copy()
...
#         self.mines.loc[:,'Sustaining CAPEX ($M)'] /= 2
        ...
        mines.loc[:,'Total reclamation cost ($M)'] = np.exp(h['primary_reclamation_constant'] +
                                                            h['primary_reclamation_slope']*np.log(mines['Capacity (kt)']/1e3))
        mines.loc[:,'Cash flow expect ($M)'] = np.nan
        mines.loc[:,'Development CAPEX ($M)'] = np.nan
    ...
    def generate_byproduct_production(self,param):
        ...
        mines = mines.loc[mines['Production fraction'].cumsum()<production_fraction,:]
        mines.loc[mines.index[-1]+1,'Production fraction'] = production_fraction-mines['Production fraction'].sum()
        mines.loc[:,'Production (kt)'] = mines['Production fraction']*h['byproduct_production']
        ...
        for i in regions:
            ...
            mines.loc[ind,'Region'] = int_version
        mines.loc[mines.Region.isna(),'Region'] = int(h[regions].astype(float).idxmax().replace('production_frac_region',''))
        ...
        mines.loc[:,'Commodity price (USD/t)'] = h[by_param+'_commodity_price']
    ...
    def correct_byproduct_production(self,param):
        ...
        for i in primary_relocate:
            ...
            if i=='Commodity price (USD/t)':
                  mines.loc[:,'Primary '+i] = mines.loc[:,i].replace(0,h['byproduct_commodity_price'])
        mines.loc[:,'Commodity price (USD/t)'] = h['byproduct_commodity_price']
...
#         mines.loc[:,'Total cash margin (USD/t)'] = mines['Commodity price (USD/t)'] - mines['Total cash cost (USD/t)']
        ...
        mines.loc[:,'Cumulative ore treated (kt)'] = mines['Cumulative ore treated ratio with ore treated']*mines['Ore treated (kt)']
        mines.loc[:,'Initial ore treated (kt)'] = mines['Ore treated (kt)']/h['ramp_up_years']
    ...
    def op_initialize_prices(self):
        ...
        for price_or_tcrc in ['price','tcrc']:
            if self.byproduct and not hasattr(self,f'primary_{price_or_tcrc}_series') and not hasattr(self,f'byproduct_{price_or_tcrc}_series'):
    ...
    def op_initialize_mine_life(self):
        ...
        if h['reinitialize']:
            ...
            # mine_life_init.loc[:,'Ramp up flag'] = 0
            ...
            mine_life_init.loc[:,'Initial price (USD/t)'] = np.nan
            ...
            if self.byproduct:
                ...
                mine_life_init.loc[:,'Primary Initial price (USD/t)'] = np.nan
    ...
    def op_simulate_mine_life(self):
        ...
        ml_yr = self.ml.loc[i] if i==simulation_time[0] else self.ml.loc[i-1]
        ml_last = ml_yr.copy()
        ml_yr.name = i
        ...
        if (ml_yr.closed_flag!=True).any():
            ...
            if self.byproduct:
                if h['incentive_tune_tcrc']:
                    ml_yr.primary_commodity_price_usdpt = ml_yr.primary_commodity_price_usdpt * (primary_price_series.pct_change().fillna(0)+1)[i]
                    ml_yr.commodity_price_usdpt = ml_yr.commodity_price_usdpt * (byproduct_price_series.pct_change().fillna(0)+1)[i]
                else:
                    ml_yr.primary_tcrc_usdpt = ml_yr.primary_tcrc_usdpt * (self.primary_tcrc_series.pct_change().fillna(0)+1)[i]
                ...
                if h['incentive_tune_tcrc']:
                    ml_yr.commodity_price_usdpt = ml_yr.commodity_price_usdpt * (primary_price_series.pct_change().fillna(0)+1)[i]
                else:
            ...
            sxew_mines = ml_yr.index[ml_yr.payable_percent_pct==100]
            ml_yr.tcrc_usdpt[sxew_mines]=0
            if self.byproduct:
                ml_yr.primary_tcrc_usdpt[sxew_mines]=0
...
#             end_ramp_up = ml_yr.loc[(opening_mines)&(ml_yr['Opening']+h['ramp_up_years']<=i)&(ml_yr['Opening']>simulation_time[0]-1)].index
                ...
                if h['ramp_up_exponent']!=0:
                    ...
                    if self.byproduct:
                        for b in ml_yr['Byproduct ID'].unique():
                            ...
                            ml_yr.capacity_utilization[by_open] = np.mean(ml0.capacity_utilization[id_zero]) * (h['ramp_up_cu']+(1-h['ramp_up_cu'])/(h['ramp_up_years'])**ramp_up_exp*(ml_yr.ramp_up_flag[by_open]-1)**ramp_up_exp)
                    else:
                        ml_yr.capacity_utilization[opening_mines] = h['mine_cu0'] * (h['ramp_up_cu']+(1-h['ramp_up_cu'])/(h['ramp_up_years'])**ramp_up_exp*(ml_yr.ramp_up_flag[opening_mines]-1)**ramp_up_exp)
                else:
            ...
            if len(end_ramp_up)>0:
                ml_yr.capacity_utilization[end_ramp_up] = self.calculate_cu(h['mine_cu0'],ml_last.total_cash_margin_usdpt[end_ramp_up],govt=False)
            ...
            ml_yr.capacity_utilization[normal_mines] = self.calculate_cu(ml_last.capacity_utilization[normal_mines],ml_last.total_cash_margin_usdpt[normal_mines])
            ...
            ml_yr.initial_ore_treated_kt[ml_yr.initial_ore_treated_kt==0] = ml_yr.ore_treated_kt[ml_yr.initial_ore_treated_kt==0]
            if i>simulation_time[0]:
            ...
            if np.sum(first_opening)>0 and h['minesite_cost_response_to_grade_price']:
                if self.byproduct:
                    for b in ml_yr['Byproduct ID'].unique():
                        id_zero = ml0.index[ml0.byproduct_id==b]
                        by_open = np.intersect1d(id_zero,opening_mines)
                        print(1501, 'could be a spot for error if ml0 does not have the right CU value')
                        ml_yr.simulation_start_ore_treated_kt[by_open] = ml_yr.ore_treated_kt[by_open]*np.mean(ml0.capacity_utilization[ml0.byproduct_id==b])/ml_yr.capacity_utilization
                else:
                    ml_yr.simulation_start_ore_treated_kt[opening_mines] = ml_yr.ore_treated_kt[opening_mines]*h['mine_cu0']/ml_yr.capacity_utilization[opening_mines]

            ml_yr.head_grade_pct = self.calculate_grade(ml_yr.initial_head_grade_pct,ml_yr.cumulative_ore_treated_kt, ml_yr.initial_ore_treated_kt, ml_yr.oge)

            if self.byproduct:
                if i!=simulation_time[0]:
                    for j in np.arange(1,4):
                        by_id = ml_yr.byproduct_id==j
                        ml_yr.recovery_rate_pct[by_id] = (ml_last.recovery_rate_pct[by_id]*(ml_last.byproduct_total_cash_margin_usdpt[by_id]/h['byproduct'+str(j)+'_mine_tcm0'])**h['byproduct_rr_margin_elas'])
                        ml_yr.recovery_rate_pct[ml_yr.recovery_rate_pct==np.nan]=-1
                        ml_yr.recovery_rate_pct[(by_id)&(ml_yr.recovery_rate_pct>h['byproduct'+str(j)+'_mine_rrmax'])] = h['byproduct'+str(j)+'_mine_rrmax']
    ...
    #                         print(h['byproduct'+str(j)+'_mine_tcm0'],h['byproduct_rr_margin_elas'])
                ml_yr.primary_head_grade_pct = self.calculate_grade(ml_yr.primary_initial_head_grade_pct,ml_yr.cumulative_ore_treated_kt, ml_yr.initial_ore_treated_kt, ml_yr.oge)
                ml_yr.primary_head_grade_pct[ml_yr.primary_head_grade_pct==np.nan]=0
                ml_yr.head_grade_pct[ml_yr.byproduct_id!=0] = ml_yr.primary_head_grade[ml_yr.byproduct_id!=0]/ml_yr.byproduct_grade_ratio[ml_yr.byproduct_id!=0]
            ...
            if self.byproduct:
                ...
                ml_yr.total_cash_margin_usdpt = (ml_yr.byproduct_total_cash_margin_usdpt*ml_yr.paid_metal_production_kt + ml_yr.primary_total_cash_margin_usdpt*ml_yr.primary_paid_metal_production_kt)/ml_yr.primary_paid_metal_production_kt
                ...
                ml_yr.cash_flow_usdm[by_only] = ml_yr.byproduct_cash_flow_usdm[by_only] - ml_yr.overhead_usdm[by_only] - ml_yr.development_capex_usdm[by_only]
                ...
                ml_yr.byproduct_revenue_fraction = ml_yr.byproduct_revenue_usdm/(ml_yr.byproduct_revenue_usdm+ml_yr.primary_revenue_usdm)
                ml_yr.byproduct_revenue_fraction[(ml_yr.byproduct_revenue_fraction>1)|(ml_yr.byproduct_revenue_fraction<0)] = np.nan
            else:
            ...
            ml_yr.price_expect_usdpt = price_expect
        ...
        self.ml_yr = ml_yr
        self.ml.add_mines(ml_yr)
        if i>self.simulation_time[0]:
            self.cumulative_ore_treated.loc[i] = self.cumulative_ore_treated.loc[i-1]+np.sum(ml_yr.ore_treated_kt)
        else:
            self.cumulative_ore_treated.loc[i] = np.sum(ml_yr.cumulative_ore_treated_kt)
...
#         if h['internal_price_formation']:
        ...
        if self.byproduct:
            self.primary_price_series.loc[i] = np.mean(ml_yr_ph.primary_commodity_price_usdpt)
            self.primary_tcrc_series.loc[i] = np.mean(ml_yr_ph.primary_tcrc_usdpt)
            self.byproduct_price_series.loc[i] = np.mean(ml_yr_ph.commodity_price_usdpt)
            self.byproduct_tcrc_series.loc[i] = np.mean(ml_yr_ph.tcrc_usdpt)
        else:
            self.primary_price_series.loc[i] = np.mean(ml_yr_ph.commodity_price_usdpt)
            self.primary_tcrc_series.loc[i] = np.mean(ml_yr_ph.tcrc_usdpt)

        self.supply_series.loc[i] = np.sum(ml_yr_ph.production_kt)
        self.concentrate_supply_series.loc[i] = np.sum(ml_yr_ph.production_kt[ml_yr_ph.payable_percent_pct!=100])
        self.sxew_supply_series.loc[i] = np.sum(ml_yr_ph.production_kt[ml_yr_ph.payable_percent_pct==100])

    def calculate_cu(self, cu_last, tcm_last, govt=True):
        neg_tcm = tcm_last<0
        cu = cu_last*abs(tcm_last/self.hyperparam['Value']['mine_tcm0'])**self.hyperparam['Value']['mine_cu_margin_elas']
        cu[neg_tcm] = 0.7
...
#             cu.loc[ind] = cu_last.loc[ind]
    ...
    def check_ramp_down(self, ml_yr_, price_df, price_expect):
        ...
        ml_yr.tcrc_expect_usdpt = tcrc_expect
        ...
        if ml_yr.shape()[0]==0 or np.sum(np.isnan(ml_yr.reserves_kt)==len(ml_yr.reserves_kt)):
            return ml_yr
        ...
        if len(neg_cash_flow)>0:
            ...
            if use_reserves_for_closure:
                reserve_violation = ml_yr.reserves_kt[neg_cash_flow] < ot_expect[neg_cash_flow]+capacity[neg_cash_flow]*self.ramp_down_cu
                reserve_violation = ml_yr.index[reserve_violation]
                ml_yr.ore_treat_ramp_following_kt[reserve_violation] = ml_yr.reserves_kt[reserve_violation]-ot_expect[reserve_violation]
                ml_yr.cu_ramp_following[reserve_violation] = ml_yr.ore_treat_ramp_following_kt[reserve_violation]/capacity[reserve_violation]
            ...
            cash_flow_ramp_next, by_cash_flow_ramp_next, tcm_rn, by_tcm_rn, _ = self.get_cash_flow(ml_yr, cumu_ot_ramp_next, ot_ramp_next, initial_ore_treated, initial_grade,
                                              ...
                                              development_capex, pri_initial_price, pri_price_expect, pri_tcrc_expect,
                                              neg_cash_flow=neg_cash_flow)
            npv_ramp_following = cash_flow_expect[neg_cash_flow] + cash_flow_ramp_following/(1+discount_rate) - reclamation/(1+discount_rate)**2
    ...
    def get_cash_flow(self, ml_yr_, cumu_ot_expect, ot_expect, initial_ore_treated, initial_grade, price_expect, tcrc_expect, initial_price, overhead, sustaining_capex, development_capex, pri_initial_price, pri_price_expect, pri_tcrc_expect, neg_cash_flow):
        ...
        ml_yr = ml_yr_.copy()
        h = self.hyperparam.copy()['Value']
        grade_expect = self.calculate_grade(initial_grade,cumu_ot_expect,initial_ore_treated,ml_yr.oge[neg_cash_flow])
        ...
        if self.byproduct:
            if self.i!=self.simulation_time[0]:
                for j in np.arange(1,4):
                    ...
                    ml_yr.recovery_rate_pct[byprod_id] = (ml_yr.recovery_rate_pct[byprod_id]*(ml_yr.byproduct_total_cash_margin_usdpt[byprod_id]/h['byproduct'+str(j)+'_mine_tcm0'])**h['byproduct_rr_margin_elas'])
                    rec_rate_exceed = ml_yr.index[ml_yr.recovery_rate_pct[byprod_id]>h['byproduct'+str(j)+'_mine_rrmax']]
                    ml_yr.recovery_rate_pct[np.intersect1d(byprod_id,rec_rate_exceed)] = h['byproduct'+str(j)+'_mine_rrmax']
            pri_grade_expect = self.calculate_grade(ml_yr.primary_initial_head_grade_pct,cumu_ot_expect,ml_yr.initial_ore_treated_kt,ml_yr.oge)
            grade_expect[ml_yr.byproduct_id!=0] = pri_grade_expect/ml_yr.byproduct_grade_ratio

#         ml_yr.loc[:,'Head grade expect (%)'] = grade_expect
        minesite_cost_expect = self.calculate_minesite_cost(ml_yr.minesite_cost_usdpt[neg_cash_flow], grade_expect, ml_yr.head_grade_pct[neg_cash_flow], price_expect, initial_price, ot_expect, ml_yr.simulation_start_ore_treated_kt[neg_cash_flow],self.i+1)
#         ml_yr.loc[:,'Minesite cost expect (USD/t)'] = minesite_cost_expect
#         ml_yr.loc[:,'Price expect (USD/t)'] = price_expect
        paid_metal_expect = ot_expect * grade_expect * ml_yr.recovery_rate_pct[neg_cash_flow] * ml_yr.payable_percent_pct[neg_cash_flow] * 1e-6
        ...
        if self.byproduct:
            ...
            pri_paid_metal_expect = ot_expect * pri_grade_expect * ml_yr.primary_recovery_rate_pct[neg_cash_flow] * ml_yr.primary_payable_percent_pct[neg_cash_flow] /1e6
            pri_minesite_cost_expect = self.calculate_minesite_cost(ml_yr.primary_minesite_cost_usdpt[neg_cash_flow], pri_grade_expect, ml_yr.primary_head_grade_pct[neg_cash_flow], pri_price_expect, pri_initial_price, ot_expect, ml_yr.simulation_start_ore_treated_kt,self.i+1)
            ...
            cash_flow_expect = by_cash_flow_expect + 1e-3*pri_paid_metal_expect*pri_tcm_expect - ml_yr.primary_sustaining_capex_usdm[neg_cash_flow] - overhead - development_capex
    ...
    def initialize_incentive_mines(self):
        ...
        if self.byproduct:
            ...
            incentive_mines.loc[:,'Primary Commodity price (USD/t)'] = pri_price_expect
            incentive_mines.loc[:,'Commodity price (USD/t)'] = price_expect
            inc.hyperparam.loc['byproduct_commodity_price'] = np.mean(price_expect)
            inc.hyperparam.loc['primary_commodity_price'] = np.mean(pri_price_expect)
            ...
            incentive_mines.loc[:,'Commodity price (USD/t)'] *= price_expect/incentive_mines['Commodity price (USD/t)'].mean()
            inc.hyperparam.loc['primary_commodity_price'] = price_expect

        grade_decline = (self.cumulative_ore_treated[i]/self.cumulative_ore_treated[self.simulation_time[0]])**h['initial_ore_grade_decline']
        cost_improve = (1-h['incentive_mine_cost_improvement']/100)**(i-self.simulation_time[0])
        incentive_mines.loc[:,'Ramp up flag'] = 1
        incentive_mines.loc[:,'Opening'] = i
        incentive_mines.loc[:,'Head grade (%)'] = incentive_mines['Initial head grade (%)'] * grade_decline
        incentive_mines.loc[incentive_mines['Head grade (%)']>80,'Head grade (%)'] = 80
        incentive_mines.loc[:,'Cumulative ore treated (kt)'] = incentive_mines['Initial ore treated (kt)']
        incentive_mines.loc[:,'Total cash margin (USD/t)'] *= h['ramp_up_cu']/incentive_mines['Capacity utilization']/cost_improve
        incentive_mines.loc[:,'Minesite cost (USD/t)'] *= h['ramp_up_cu']/incentive_mines['Capacity utilization']*cost_improve
        incentive_mines.loc[:,'Capacity utilization'] = h['ramp_up_cu']
        if self.verbosity>4:
            print(1951,'incentive tcrc mean',incentive_mines['TCRC (USD/t)'].mean())
        if incentive_mines['TCRC (USD/t)'].mean()!=0:
            incentive_mines.loc[:,'TCRC (USD/t)'] *= tcrc_expect/incentive_mines['TCRC (USD/t)'].mean()
...
#         incentive_mines.loc[:,'Commodity price (USD/t)'] *= self.ml['Commodity price (USD/t)'].unstack(0)[i].mean()/incentive_mines['Commodity price (USD/t)'].mean()
        ...
        if self.byproduct:
            ...
            incentive_mines.loc[:,'Primary Total cash margin (USD/t)'] *= h['ramp_up_cu']/incentive_mines['Capacity utilization']/cost_improve
            incentive_mines.loc[:,'Byproduct Total cash margin (USD/t)'] *= h['ramp_up_cu']/incentive_mines['Capacity utilization']/cost_improve
            incentive_mines.loc[:,'Primary Minesite cost (USD/t)'] *= h['ramp_up_cu']/incentive_mines['Capacity utilization']*cost_improve
        ...
        if i-1 in self.demand_series.index and (i-1 not in self.resources_contained_series.index or self.resources_contained_series.isna()[i-1]):
            self.resources_contained_series.loc[i-1] = self.demand_series[i-1]*h['annual_reserves_ratio_with_initial_production_const']
        if i>self.simulation_time[0] and h['reserves_ratio_price_lag']>i-self.simulation_time[0]-1:
            ...
            self.resources_contained_series.loc[i] = self.resources_contained_series[i-1]*\
                 self.demand_series[i]/self.demand_series[i-1]*\
                 (1+h['annual_reserves_ratio_with_initial_production_slope'])*\
                 (self.primary_price_series[i-lag]/self.primary_price_series[i-lag-1])**h['primary_price_resources_contained_elas']
            if self.byproduct:
                self.resources_contained_series.loc[i] = self.resources_contained_series[i-1]*\
                     self.demand_series[i]/self.demand_series[i-1]*\
                     (1+h['annual_reserves_ratio_with_initial_production_slope'])*\
                     (self.byproduct_price_series[i-lag]/self.byproduct_price_series[i-lag-1])**h['byproduct_price_resources_contained_elas']

        max_res = self.demand_series[self.simulation_time[0]]*h['annual_reserves_ratio_with_initial_production_const']*100
...
#                 incentive_mines = pd.DataFrame(incentive_mines.iloc[0]).T.rename({0:i})
        ...
        self.perturb_cols=[i for i in incentive_mines.loc[:,incentive_mines.dtypes==float].columns if 'price' not in i and i not in ['Opening','Closure','Known opening','Region','Mine type','Risk indicator']]
    ...
    def add_development_capex(self):
        ...
        inc_mines.loc[:,'Development CAPEX ($M)'] = dcapex/h['ramp_up_years']
...
#                                        self.ml.index.get_level_values(1).max()+self.i*1e4+mines_to_open.shape[0]))),level=1)
    ...
    def incentive_open_xinkai_thesis(self):
        ...
        condition_for_ml = i>self.simulation_time[0]+h['ml_accelerate_initialize_years'] and i%h['ml_accelerate_every_n_years']!=0
        ...
        current_prod = np.sum(outside_ml.production_kt)
        inc_mines = opening_sim.loc[i].copy()
        opening_sim.loc[:,'Discount rate (divisor)'] = [(1+h['incentive_discount_rate'])**(j[0]-simulation_time[0]) for j in opening_sim.index]
        inc_mines.loc[:,'NPV ($M)'] = (opening_sim['cash_flow_usdm']/opening_sim['Discount rate (divisor)']).groupby(level=1).sum()
...
#                 opener_predict_knc, self.accuracy_knc.loc[i] = run_mine_ml_model(KNeighborsClassifier, self.all_inc_mines, inc_mines, verbosity=0)
                ...
                opener_predict_lda, self.accuracy_lda.loc[i] = run_mine_ml_model(LinearDiscriminantAnalysis, self.all_inc_mines, inc_mines, verbosity=0)
...
#                 opener_predict_dtc, self.accuracy_dtc.loc[i] = run_mine_ml_model(DecisionTreeClassifier, self.all_inc_mines, inc_mines, verbosity=0)
            if run_ml_selected_mines and condition_for_ml:
                ...
                opening_sim2.loc[:,'Discount rate (divisor)'] = [(1+h['incentive_discount_rate'])**(j[0]-simulation_time[0]) for j in opening_sim2.index]
                inc2_mines.loc[:,'NPV ($M)'] = (opening_sim2['cash_flow_usdm']/opening_sim2['Discount rate (divisor)']).groupby(level=1).sum()
        ...
        if h['calibrate_incentive_opening_method'] or (h['incentive_opening_method']=='unconstrained' and i<=self.end_calibrate and h['incentive_opening_probability']==0) or self.i<=h['incentive_require_tune_years']+self.simulation_time[0]:
            ...
            error = subset.loc[subset['NPV ($M)']>0,'production_kt'].sum() + current_prod - target
            if h['ramp_up_exponent']!=0:
                error /= (h['ramp_up_years']+1)**(h['ramp_up_exponent'])
            error_series = pd.Series(error,[n])
            ...
            n_mod=1
            not_sign = not sign
#             if ((subset['NPV ($M)']>0).sum()>0 or True) and self.ml.loc[i,'Production (kt)'].sum() < target:
            while (sign or not_sign) and n>0:
                n -= n_mod if sign else -1*n_mod
                subset = inc_mines.sample(n=n, random_state=self.rs, replace=n>=inc_mines.shape[0])
                error = subset.loc[subset['NPV ($M)']>0,'production_kt'].sum() + current_prod - target
                if h['ramp_up_exponent']!=0:
                    error /= (h['ramp_up_years']+1)**(h['ramp_up_exponent'])
#                 print(n,'\t',error,'\n\tnew: {:.3f}, previous: {:.3f}, target: {:.3f}'.format(subset.loc[subset['NPV ($M)']>0,'Production (kt)'].sum(), self.ml.loc[i,'Production (kt)'].sum(), target))
#                 if (subset['NPV ($M)']>0).sum()>0:
                error_series.loc[n] = error
                sign = error>0
                if sign == not_sign:
                    break
                elif n%1e3==0 and n>0:
                    n_mod *=2
                elif n>1e4:
                    if self.verbosity>-1:
                        print('perhaps runaway demand or mines are incapable of opening (low ore grade likely culprit)\n\tSet verbosity=(-1) to remove this message')
                    break
#             elif self.ml.loc[i,'Production (kt)'].sum() > target:
#                 print(2047, 'production already exceeds demand')
#             else:
#                 print(2049, 'no profitable mines found')
        ...
        if self.verbosity>1:
            ...
            print('Number of mines closing:',np.sum(self.ml_yr.closed_flag))
        self.inc = inc
    ...
    def incentive_open_karan_generalization(self):
        ...
        if h['calibrate_incentive_opening_method']:
            ...
            self.all_inc_mines = pd.DataFrame()
            count = 0
            while (abs(error)>target*mpc_multiplier or target_new==0) and (price_change>min_price_change or error==min_error):
                if target_new<subset['Production (kt)'].sum():
                    inc_mines.loc[:,price_select] *= (price + multiplier*price_change)/inc_mines[price_select].mean()
                elif incentive_tune_tcrc:
                    inc_mines.loc[:,price_select] = 10
                ...
                if count >=2:
                    opener_predict_knc, accuracy = run_mine_ml_model(KNeighborsClassifier, self.all_inc_mines, subset, verbosity=3)
                ...
                if sign == not_sign and price_change != min_price_change:
                    ...
                    if price_change<min_price_change:
                        ...
                        if (error==min_error_series.iloc[0] or target-operating_production==min_error_series.iloc[0]) and no_mines:
                            error_series.loc[min_error_series.index] = 0
                            print('line 2098 executed')
                            min_error_series.loc[:] = 0
                        if (incentive_tune_tcrc and target_new!=0 and no_mines==False) or (target_new==0 and no_mines and incentive_tune_tcrc==False):
                ...
                elif inc_mines.loc[:,'Commodity price (USD/t)'].mean()>1e6:
            ...
            elif (incentive_tune_tcrc and target_new!=0 and no_mines==False) or (no_mines and incentive_tune_tcrc==False):
    ...
    def run_karan_generalization(self,price):
        ...
        s.simulate_mine_life_all_years()
        opening_sim = s.ml.copy()
        opening_sim.loc[:,'Discount rate (divisor)'] = [(1+h['discount_rate'])**(j[0]-simulation_time[0]) for j in opening_sim.index]
        inc_mines.loc[:,'NPV ($M)'] = (opening_sim['Cash flow ($M)']/opening_sim['Discount rate (divisor)']).groupby(level=1).sum()
    ...
    def bayesian_tune(self):
        ...
        for i in np.arange(0,20):
            ...
            if self.verbosity > 3:
                print('  x = {:.3f}, predicted = {:.3f}, actual = {:.3f}, rescaled price: {:.1f}\nCurrent best: {:.1f}:'.format(x,est[0][0],actual,x_rescaled,X[:,0][np.argmin(abs(y))]*scale))
    ...
    def update_price_tcrc(self, byproduct=False):
        ...
        if h['internal_price_formation']:
            ...
            if h['incentive_opening_method'] == 'karan_generalization':
                ml_ot = self.ml.loc[idx[self.i,:],'Ore treated (kt)']
                ml_price = self.ml.loc[idx[self.i,:],price_select]
                open_ot = opening['Ore treated (kt)']
                open_price = opening[price_select]*self.new_price_select/opening[price_select].mean()
                new_price_select = ((ml_ot*ml_price).sum() + (open_ot*open_price).sum())/(ml_ot.sum()+open_ot.sum())
    #             new_price_select = open_price.mean()
                ...
                if 'pid-' in incentive_tuning_option:
                    if i<int(incentive_tuning_option.split('-')[1]): incentive_tuning_option = 'pid'
                    else: incentive_tuning_option = 'elas'
                if (i<self.end_calibrate and h['incentive_opening_probability']==0) or incentive_tuning_option=='elas':
                    ...
                    mn = self.ml['TCRC (USD/t)'].unstack(0)[i].mean()
                    k_u = 1
                    t_u = 20
                    ...
                    if not hasattr(self,'pid_params'):
                        self.pid_params = pd.DataFrame(np.nan, self.simulation_time, ['error','integral','derivative'])
                ...
                if self.verbosity>3:
                    ...
                    pd.concat([self.ml[price_select],opening.loc[:,price_select]]).unstack(0).mean().plot(ax=ax[2],marker='o')
            ...
            for price_select,alt_select in zip(['TCRC (USD/t)','Commodity price (USD/t)'],['tcrc_usdpt','commodity_price_usdpt']):
                if byproduct:
                    ...
                    if 'TCRC' in price_select:
                         setattr(self.ml.loc[i],alt_select, getattr(self.ml.loc[i],alt_select)*new_price/np.mean(getattr(self.ml.loc[i],alt_select)))
                    else:
                        setattr(self.ml.loc[i],alt_select,new_price)
                    if opening.loc[:,alt_select].mean()!=0:
                        opening.loc[:,alt_select] *= new_price/opening.loc[:,alt_select].mean()
                    else:
                        opening.loc[:,alt_select] = new_price
    ...
    def simulate_history(self):
        ...
        sh.primary_price_series = pd.Series(h['primary_commodity_price'],sh.simulation_time)
        if self.byproduct: sh.byproduct_price_series = pd.Series(h['byproduct_commodity_price'],sh.simulation_time)
