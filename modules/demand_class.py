...
class demandModel():
    ...
    def __init__(self, static_data_folder=None, user_data_folder=None, simulation_time=np.arange(2019,2041), verbosity=0):
        ...
        file = f'{self.static_data_folder}/Demand prediction data-copper.xlsx'
    ...
    def load_demand_data(self):
        ...
        self.alt_demand = pd.read_excel(f'{self.static_data_folder}/End use combined data-copper.xlsx',sheet_name='Combined',index_col=0)
        intensity_parameters_cu = pd.read_excel(f'{self.static_data_folder}/elasticity estimates-copper.xlsx', sheet_name='S+R S intercept only', header=[0], index_col=0).sort_index(axis=1)
        self.intensity_parameters_al = pd.read_excel(f'{self.static_data_folder}/baseline_scenario_aluminum.xlsx', sheet_name='intensity_parameters', header=[0,1], index_col=0).sort_index().sort_index(axis=1)
        ...
        if self.hyperparam['Value']['commodity'] in ['Au','Ag']:
            gold_vols = pd.read_excel(f'{self.static_data_folder}/Gold demand volume indicators.xlsx',sheet_name='Volume drivers',index_col=0).loc[2001:]
            ...
            gold_dem = pd.read_excel(f'{self.user_data_folder}/case study data.xlsx',sheet_name=self.hyperparam['Value']['commodity'],index_col=0).loc[2001:,'Total demand'].astype(float)
            ...
            gold_vols = pd.read_excel(f'{self.static_data_folder}/Gold demand volume indicators.xlsx',
            ...
            gold_dem = pd.read_excel(f'{self.user_data_folder}/case study data.xlsx',
